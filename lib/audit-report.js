...
async function showAuditReport(repoRoot, limit = 50) {
  ...
  if (expired.length > 0) {
    ...
    for (const e of expired.slice(-10)) {
      console.log(`  ${formatLocalDate(e.timestamp)}  ${e.rule_id}  ${stripControl(e.file)}:${e.line}`);
  ...
  if (blocked.length > 0) {
    ...
    for (const e of blocked.slice(-10)) {
      ...
      console.log(`  ${DIM}${stripControl(e.file)}:${e.line}  [${stripControl(e.git_user)}]${RESET}`);
    }
  }
  ...
  if (excepted.length > 0) {
    ...
    for (const e of excepted.slice(-20)) {
      ...
      console.log(`  ${DIM}${date}${RESET}  ${YELLOW}${e.rule_id}${RESET}  ${stripControl(e.file)}`);
