...
function getGitUser() {
  try {
    ...
    // A hostile repo's local .git/config controls these — strip terminal control sequences
    // so a crafted author name can't drive the terminal via audit output or insights.
    const name  = stripControl(execSync('git config user.name',  { encoding: 'utf8' }).trim());
    const email = stripControl(execSync('git config user.email', { encoding: 'utf8' }).trim());
    return { name, email };
  } catch {
...
function logScan(repoRoot, { hookType, files, findings, blocked, exceptions_applied, scanId, noSync, scanMode, repoContext, repoContextChanged, scopeExclusions = null }) {
  ...
  for (const f of findings) {
    ...
    logEvent(repoRoot, eventType, {
      ...
      file:         f.filePath,
      line:         f.line,
      code_hash:    f.codeHash  || null,   // content identity — required to reconstruct via sync --history
  ...
  if (!noSync) {
    try {
      ...
      if (centralUrl) {
        ...
        const repoId = store.getRepoId(repoRoot);   // attribution — stamped on every queued event
