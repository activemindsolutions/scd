...
function logScan(repoRoot, { hookType, files, findings, blocked, exceptions_applied, scanId, noSync, scanMode }) {
  ...
  // Falls back to generating a random ID if called without one (e.g. from hooks).
   console.log('[debug-logScan] called, scanId:', scanId, new Error().stack.split('\n')[2]);
