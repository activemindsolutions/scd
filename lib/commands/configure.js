...
function register(program) {
  program
    ...
    .option('--unset <key>',            'Remove a global default so it inherits the built-in default (e.g. --unset trust-level)')
    .action((opts) => {
      ...
      // Multiple setter options can be combined in one call (e.g. --central-url
      // with --token). Each setter marks handled instead of exiting, so they all
      // run; we exit after them, falling through to --show only when none were given.
      let handled = false;
      ...
      if (opts.token) {
        ...
        console.log(`  ${DIM}Next: run ${RESET}${CYAN}scd scan${RESET}${DIM} in a repository — the first scan activates this installation on the server.${RESET}\n`);
        handled = true;
      }
      ...
      // ── --unset <key> (fall back to the built-in default) ─────────────────
      // Hyphenated key (e.g. trust-level) for consistency with the flags.
      if (opts.unset !== undefined) {
        ...
        if (!REPO_KEYS.includes(key)) {
          const valid = REPO_KEYS.map(k => k.replace(/_/g, '-')).join(' | ');
          console.error(`\n${RED}✗ Unknown setting '${dashKey}'. Use one of: ${valid}${RESET}\n`);
          process.exit(1);
        }
        const gcKey   = 'REPO_' + key.toUpperCase();
        const gc2     = require('../global-config');
        const existed = gc2.get(gcKey) !== undefined;
        gc2.remove(gcKey);
        const def = String(require('../config').DEFAULTS[key]);
        if (existed) {
          console.log(`\n${GREEN}✓ Global default ${dashKey} unset${RESET} — now using the built-in default: ${BOLD}${def}${RESET}\n`);
        } else {
          console.log(`\n${DIM}Global default ${dashKey} was not set — already using the built-in default: ${def}${RESET}\n`);
        }
        handled = true;
      }

      // A setter ran — done. Fall through to --show only when no setter was given.
