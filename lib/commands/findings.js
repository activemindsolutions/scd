...
async function findingsAction(findingId, opts) {
      ...
      const { sevConfig, sevOrder, SEVERITY } = require('../severity');
      const { sourceLink } = require('../terminal-links');
      const { stripControl } = require('../sanitize');

      // Validate --severity up front. An unknown/misspelled value must not silently filter
      // to zero findings and then render a green "✓ No open findings" all-clear — that is a
      // false all-clear in a security tool.
      if (opts.severity && !SEVERITY[opts.severity.toUpperCase()]) {
        console.error(`\n${RED}✗ Unknown severity: ${opts.severity}${RESET}`);
        console.error(`${DIM}   Use one of: critical, high, medium, exposure, info${RESET}\n`);
        process.exit(1);
      }
      ...
      if (opts.showSuppressed) {
        ...
        for (const [filePath, fileFindings] of Object.entries(byFile).sort()) {
          console.log(`  ${BOLD}${stripControl(filePath)}${RESET}`);
