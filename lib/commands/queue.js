...
function entryLine(e) {
  ...
  const flagged  = e.stale || e.authBlocked;
  const marker   = flagged ? (YELLOW + '⚠' + RESET) : (GREEN + OK + RESET);
  const staleTag = e.stale       ? '  ' + YELLOW + 'stale: ' + e.reason + RESET : '';
  const authTag  = e.authBlocked ? '  ' + YELLOW + 'blocked: token rejected' + RESET : '';
  return '  ' + marker + '  ' +
    type.padEnd(18) +
    DIM + formatLocalTime(e.ts).padEnd(22) + RESET +
    'attempts: ' + e.attempts +
    staleTag + authTag;
}
...
function renderList(opts = {}) {
  ...
  const authBlocked = entries.filter(e => e.authBlocked);
  ...
  // #67: a rejected token is a PERMANENT block (not a transient retry) — surface
  // it prominently so the queue never looks healthy while delivery is dead.
  if (authBlocked.length > 0) {
    console.log('  ' + YELLOW + '⚠  Delivery is blocked — the server is rejecting your token' + RESET +
      DIM + ' (permanent until fixed).' + RESET);
    console.log('     ' + DIM + 'Re-create the token on the server, then: ' + RESET +
      CYAN + 'scd configure --token <token>' + RESET);
    console.log(DIM + SEP + RESET);
  }
