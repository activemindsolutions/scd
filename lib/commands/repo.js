...
function register(program) {
  ...
  const repoConfigureCmd = new Command('configure')
    ...
    .option('--unset <key>',               'Remove a per-repo setting so it inherits global/default (e.g. --unset trust-level)')
    .action((opts) => {
      const fs         = require('fs');
      ...
      // If no config exists yet, create it on demand (in the global store, never in
      // the repo) so a missing config never blocks configure — matching `scd scan`,
      // which also works without an explicit `scd init`.
      function updateConfigYml(key, value) {
        if (!fs.existsSync(configPath)) {
          const { ensureRepoConfig } = require('../init-repo');
          ensureRepoConfig(repoRoot);
          console.log(`\n${DIM}No repo config yet — created ${configPath}${RESET}`);
        }
      ...
      // Helper: remove a key's line from config.yml so its value inherits from the
      // layer below (global → default). Returns whether a line was actually removed.
      // Only matches an active line (a commented `# key:` is left untouched).
      function removeConfigYml(key) {
        if (!fs.existsSync(configPath)) return false;
        const content = fs.readFileSync(configPath, 'utf8');
        const re = new RegExp(`^${key}:\\s*.*(?:\\r?\\n)?`, 'm');
        if (!re.test(content)) return false;
        fs.writeFileSync(configPath, content.replace(re, ''), 'utf8');
        return true;
      }

      // ── unset operation (fall back to global/default) ────────────────────
      // The user types the hyphenated key (e.g. trust-level), matching the flag
      // names; translate to the underscore config key.
      if (opts.unset !== undefined) {
        const dashKey = opts.unset.trim();
        const key     = dashKey.replace(/-/g, '_');
        if (!KEYS.includes(key)) {
          const valid = KEYS.map(k => k.replace(/_/g, '-')).join(' | ');
          console.error(`\n${RED}✗ Unknown setting '${dashKey}'. Use one of: ${valid}${RESET}\n`);
          process.exit(1);
        }
        const removed  = removeConfigYml(key);
        const eff      = String(yaml.loadConfig(repoRoot)[key]);
        const src      = yaml.configSource(repoRoot, key);
        const srcLabel = src === 'repo' ? "this repo's config"
                       : src === 'global' ? 'your global config'
                       : 'the built-in default';
        if (removed) {
          console.log(`\n${GREEN}✓ ${dashKey} unset for this repo${RESET} — now: ${BOLD}${eff}${RESET} ${DIM}(from ${srcLabel})${RESET}\n`);
        } else {
          console.log(`\n${DIM}${dashKey} was not set for this repo — already inheriting: ${eff} (from ${srcLabel})${RESET}\n`);
        }
        return;
      }
  ...
  // ── scd repo hooks ────────────────────────────────────────────────────────
  // Verb subcommands (disable / enable / show) are the canonical form. The old
  // --disable / --enable flags are kept but DEPRECATED — they still perform the
  // action, but print a deprecation notice pointing at the verb. Tracked for
  // removal in the deprecation-cleanup card.

  function hooksDisable(repoRoot, reason) {
    const { getHookStatus, disableHooks } = require('../hooks-manager');
    ...
    try {
      disableHooks(repoRoot, reason);
      logHooks(repoRoot, { action: 'disable', reason: reason.trim(), noSync: false });
  ...
  function hooksEnable(repoRoot) {
    const { getHookStatus, enableHooks } = require('../hooks-manager');
    const { logHooks } = require('../audit');
    const current = getHookStatus(repoRoot);
  ...
  function hooksShow(repoRoot) {
    ...
    const statusLabel = status.status === 'enabled'
      ...
      : status.status === 'disabled'
        ...
        : status.status === 'shadowed'
          ? `${RED}shadowed (core.hooksPath points elsewhere)${RESET}`
    ...
    if (status.status === 'disabled') {
      console.log(`  ${DIM}Re-enable with: scd repo hooks enable${RESET}`);
    } else if (status.status === 'shadowed') {
      console.log(`  ${RED}scd hooks do NOT run — core.hooksPath points to a foreign directory.${RESET}`);
      console.log(`  ${DIM}Fix (${status.source}): ${status.source === 'local'
        ? 'git config --unset core.hooksPath'
        : 'git config --global core.hooksPath ~/.scd/hooks'}${RESET}`);
  ...
  function deprecatedHookFlag(flag, verb) {
    console.log(`${YELLOW}⚠  --${flag} is deprecated${RESET}${DIM} — use ${RESET}${CYAN}scd repo hooks ${verb}${RESET}${DIM} instead.${RESET}`);
  }

  const repoHooksCmd = new Command('hooks')
    .description('Show or manage git hook status for the current repo')
    .option('--disable', '(deprecated — use: scd repo hooks disable) disable git hooks; requires --reason')
    .option('--enable',  '(deprecated — use: scd repo hooks enable) re-enable git hooks')
    .option('--reason <text>', 'Reason when disabling hooks (logged to the audit trail)')
    .action((opts) => {
      const repoRoot = getRepoRoot();
      if (opts.disable) { deprecatedHookFlag('disable', 'disable --reason "..."'); return hooksDisable(repoRoot, opts.reason); }
      if (opts.enable)  { deprecatedHookFlag('enable', 'enable'); return hooksEnable(repoRoot); }
      hooksShow(repoRoot);
    });

  repoHooksCmd
    .command('disable')
    .description('Disable git hooks for this repo (requires --reason)')
    .option('--reason <text>', 'Required reason, logged to the audit trail')
    .action((opts) => hooksDisable(getRepoRoot(), opts.reason));

  repoHooksCmd
    .command('enable')
    .description('Re-enable git hooks for this repo')
    .action(() => hooksEnable(getRepoRoot()));

  repoHooksCmd
    .command('show')
    .description('Show git hook status for this repo (default action)')
    .action(() => hooksShow(getRepoRoot()));
    ...
    .action(async (opts) => {
      ...
      if (false) {
        ...
        for (const s of scans) {
          const date    = s.scanDate ? formatLocalTime(s.scanDate) : '—';
...
Examples:
  ...
  For global (all repos) scope: scd scope --show`)
    ...
    .action(async (opts) => {
      ...
      // Propagate the repo's scope to scd-server so it can shed / suppress the
      // findings the CLI now excludes (#239). Offline-first + quiet on failure.
      const syncScope = async () => {
        const r = await require('../scope-sync').pushScope(repoRoot);
        if (r.sent && (r.scoped || r.unscoped)) {
          console.log(`${DIM}  scd-server: ${r.scoped} finding(s) scoped, ${r.unscoped} un-scoped.${RESET}\n`);
        }
      };
      ...
      if (opts.removeFile) {
        ...
        if (removed.length === 0) {
          ...
          for (const r of removed) {
            ...
            console.log(`  ${DIM}Added by: ${r.added_by || '(unknown)'}  ${formatLocalTime(r.added_at)}${RESET}`);
          }
          console.log();
        }
        await syncScope();
