...
async function generateReportFile(repoRoot, opts) {
  ...
  const { target, totalFiles, skipped, scanDate, deepResults } = cache;
  let findings = cache.findings;

  // E1c.1: derive excepted/rejected fresh through the single gatekeeper so the
  // default report agrees with `scd findings` (derive-not-persist, never trust the
  // scan-frozen flag). A historic --scan is a frozen snapshot — it keeps its
  // scan-time exception status, matching `scd findings --scan`, which likewise does
  // not re-derive. Finding source is unchanged (still the cache); only the excepted/
  // rejected flags are reconciled against the current exceptions.
  if (!opts.scan) {
    try {
      const { loadExceptionsWithBootstrap } = require('../exceptions-store');
      const { reconcileException }          = require('../exception-gatekeeper');
      // Run 2 read-truth: exceptions from the machine-local store (bootstrap-on-read
      // migrates config.yml → exceptions.jsonl on first read).
      const exceptions = loadExceptionsWithBootstrap(repoRoot).records;
      findings = findings.map(f => {
        const r = reconcileException(f, exceptions);
        return { ...f, excepted: r.excepted, rejected: r.rejected };
      });
    } catch { /* non-fatal — fall back to cached excepted values */ }
  }

  const age    = cacheAge(scanDate);
  const deepMap = deepResults ? new Map(deepResults) : null;

  console.log('\n' + DIM + '↺ Using cached scan from ' + age + ' (' + formatLocalTime(scanDate) + ')' + RESET);
