...
function register(program) {
  ...
  program
    ...
    .action(async (targets, opts) => {
      ...
      const { loadConfig, getRepoRoot, configSource } = require('../config');
      ...
      // `--format json` to stdout must be a clean, parseable document. Route all human/
      // status output (banner, progress, warnings — from this command and the modules it
      // calls) to stderr for the duration; the JSON is written to stdout explicitly below.
      // With `--output <file>` the JSON goes to the file, so stdout stays human-readable.
      const jsonStdout = opts.format === 'json' && !opts.output;
      const _origConsoleLog = console.log;
      if (jsonStdout) console.log = (...args) => process.stderr.write(args.join(' ') + '\n');
      ...
      if (opts.hook) {
        ...
        let files = await getChangedFiles(opts.hook);
        if (files.length === 0) {
          console.log(DIM + '[scd] No files to scan.' + RESET);
          process.exit(0);
        }

        // Honour scope exclusions in hook mode too — files/dirs the operator
        // excluded via `scd repo scope` are skipped here just as in a manual
        // scan. Without this the hook scanned excluded (and non-source data)
        // files anyway, which could produce thousands of findings and stall the
        // scan. Same helper + filter the manual path uses.
        let scope = null;
          ...
          try {
            ...
            if (scope.file_excludes.length > 0) {
              ...
              files = files.filter(f => {
                ...
                if (isFileExcluded(scope, abs, repoRoot).excluded) { excludedCount++; return false; }
                return true;
              });
              if (excludedCount > 0) {
                scopeExclusions = {
                  ...
                  _summary:       summariseScope(scope),
                  _warnings:      scopeWarnings,
                };
              }
            }
          } catch { /* non-fatal — scope unavailable, scan proceeds unfiltered */ }
        }

        if (files.length === 0) {
          console.log(DIM + '[scd] No files to scan (all changed files are excluded by scope).' + RESET);
          process.exit(0);
        }
        ...
        // scope + scopeExclusions were resolved above — scope exclusions are now
        // honoured in hook mode (file_excludes filtered; rule_excludes passed below).
        ...
        const findings = opts.hook === 'pre-commit'
          ...
          : await scanFull(files, config, scope);
        ...
        // Scope is now loaded in hook mode (above), so the rule-exclude guard applies.
        ...
        // Persist a scan file for hook scans too: write on findings by default so
        // `scd report`, `--scan`, and the scan history can reach hook scans; empty
        // hook scans are written only when write_empty_hook_scans is enabled. This is
        // the scan-file artifact decision — separate from the findings.jsonl diff, which
        // already ran above. Same scanId cross-references the audit log + store rows.
        if (findings.length > 0 || config.write_empty_hook_scans) {
          saveCache(repoRoot, {
            findings,
      ...
      // Every value-taking option on `scd scan` must be listed here, or its value is
      // mis-parsed as a scan target (silent bug). Keep in sync with the .option() calls above.
      const knownOptionFlags = new Set([
        '--hook', '--lang', '--severity', '--rule', '--format', '--output', '--exclude', '--exclude-rule', '--log-to', '--max-findings', '--deep-delay'
      ]);
      ...
      if (opts.format === 'json') {
        ...
        if (opts.output) {
          ...
          console.log = _origConsoleLog;     // restore — stdout carries only the JSON
      ...
      if (opts.deep) {
        ...
        if (config.trust_level === 'maximum_privacy') {
          const src      = configSource(repoRoot, 'trust_level');
          const srcLabel = src === 'repo' ? "this repo's config" : 'your global config';
          console.log('\n' + YELLOW + `⚠  --deep is disabled — trust_level is maximum_privacy (from ${srcLabel}).` + RESET);
          console.log(DIM + '   Enable deep analysis for this repo: ' + RESET + CYAN + 'scd repo configure --trust-level balanced' + RESET + '\n');
        } else {
          ...
          if (deepResults && deepResults.size > 0) {
            ...
            if (deepSrc) {
              ...
              // Data jurisdiction — where the provider processes the request.
              // Known codes get a readable label; anything else prints as-is.
              const jurLabels = { 'EU-SE': 'EU-SE (Sweden)', 'US': 'US', 'customer-infra': 'your infrastructure' };
              const jurNote  = deepSrc.data_jurisdiction
                ? `  ·  jurisdiction: ${jurLabels[deepSrc.data_jurisdiction] || deepSrc.data_jurisdiction}`
                : '';
              console.log(`${DIM} [AI] ${deepSrc.provider}${modelStr}${envNote}${jurNote}${RESET}`);
            }
            // Token spend for this deep run (when the server reports it).
            const tu = deepResults._tokenUsage;
            if (tu && tu.total_tokens) {
              const reasoningStr = tu.reasoning_tokens ? `, ${tu.reasoning_tokens} reasoning` : '';
              console.log(`${DIM}       ${tu.total_tokens.toLocaleString()} tokens (${tu.input_tokens} in / ${tu.output_tokens} out${reasoningStr})${RESET}`);
            }
          }
