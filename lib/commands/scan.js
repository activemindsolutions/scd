  ...
  program
    ...
    .action(async (targets, opts) => {
      ...
      if (opts.hook) {
        ...
        const storeResult = updateFindings(repoRoot, findings, { scanId });
        ...
        console.log(`${DIM}  Open issues: ${storeResult.total} (added: ${storeResult.added}, refreshed: ${storeResult.refreshed})${RESET}`);
      ...
      if (!skipLogging) {
        const { updateFindings } = require('../findings-store');
        storeResult = updateFindings(repoRoot, scannerFindings, { scanId });
      }

      if (process.stderr.isTTY) process.stderr.write('\r\x1b[K'); // clear saving status

      // allFindings: full set → audit log + cache; displayFindings: scoped → output + deep
