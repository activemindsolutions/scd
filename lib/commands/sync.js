...
function register(program) {
  program
    ...
    .action(async (opts) => {
      ...
      // Propagate the repo's scope so the server sheds / suppresses findings the
      // CLI now excludes (#239). Quiet unless it changed something server-side.
      try {
        const sc = await require('../scope-sync').pushScope(repoRoot);
        if (sc.sent && (sc.scoped || sc.unscoped)) {
          console.log(`\n${DIM} Scope reconciliation:${RESET}`);
          console.log(`${DIM}   ${sc.scoped} finding(s) scoped, ${sc.unscoped} un-scoped on the server.${RESET}`);
        }
      } catch { /* offline-first — quiet */ }
...
// (no news is good news); loud only for the healed / needs-attention / skipped cases.
function printReconcileSummary(rc) {
  if (!rc || (!rc.healed && !rc.reapproval_required && !rc.conflict_rejected && !rc.skipped)) return;
  ...
  if (rc.skipped) {
    const parts = [];
    if (rc.skippedScoped) parts.push(`${rc.skippedScoped} scope-excluded`);
    if (rc.skippedGone)   parts.push(`${rc.skippedGone} file removed`);
    console.log(`${DIM}   ${rc.skipped} exception(s) not sent (${parts.join(', ')})${RESET}`);
    if (rc.skippedGone) {
      console.log(`${DIM}     "file removed" = the file was renamed/deleted; the local exception is stale.${RESET}`);
    }
  }
}
