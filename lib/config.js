...
/**
 * Where the effective trust_level comes from, for user-facing messaging:
 *   'repo'    – pinned in this repo's config.yml
 *   'global'  – ~/.scd/config (REPO_TRUST_LEVEL)
 *   'default' – code default (never maximum_privacy)
 * Mirrors loadConfig's priority (repo > global > default) for this single key.
 */
function trustLevelSource(repoRoot) {
  const store      = require('./store');
  const configPath = store.configPath(repoRoot);
  if (fs.existsSync(configPath)) {
    try {
      const parsed = parseSimpleYaml(fs.readFileSync(configPath, 'utf8'));
      if (parsed.trust_level !== undefined) return 'repo';
    } catch { /* fall through to global */ }
  }
  try {
    const gc = require('./global-config');
    if (gc.get('REPO_TRUST_LEVEL') !== undefined) return 'global';
  } catch { /* global-config unavailable */ }
  return 'default';
}

module.exports = { loadConfig, getRuleAction, getRepoRoot, CONFIG_FILENAME, trustLevelSource };
