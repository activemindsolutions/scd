...
const DEFAULTS = {
  ...
  write_empty_hook_scans: false,  // hook scans write a scan file only on findings; true = also write empty ones
};
...
/**
 * Where the effective value of a config key comes from, for user-facing messaging:
 *   'repo'    – set in this repo's config.yml
 *   'global'  – ~/.scd/config.yml (REPO_<KEY>)
 *   'default' – code default
 * Mirrors loadConfig's priority (repo > global > default).
 */
function configSource(repoRoot, key) {
  const store      = require('./store');
  const configPath = store.configPath(repoRoot);
  if (fs.existsSync(configPath)) {
    try {
      const parsed = parseSimpleYaml(fs.readFileSync(configPath, 'utf8'));
      if (parsed[key] !== undefined) return 'repo';
    } catch { /* fall through to global */ }
  }
  try {
    const gc = require('./global-config');
    if (gc.get('REPO_' + key.toUpperCase()) !== undefined) return 'global';
  } catch { /* global-config unavailable */ }
  return 'default';
}

module.exports = { loadConfig, getRuleAction, getRepoRoot, CONFIG_FILENAME, configSource, DEFAULTS };
