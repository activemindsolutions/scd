...
function loadConfig(repoRoot) {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      console.error(`\x1b[33m[scd] Could not read config: ${err.message}\x1b[0m`);
    }
  }
...
function isExcepted(config, finding, lineContent) {
  ...
  for (const exc of config.exceptions) {
    ...
    // Hash match — two formats supported:
    // 1. codeHash (32-char hex): stored by addExceptionById from scanner finding
    // 2. hashLine() format "sha256:{16hex}": stored by legacy addException path
    // Compare exc.line_hash against both finding.codeHash and hashLine(lineContent).
    const codeHashMatches = exc.line_hash && finding.codeHash &&
      exc.line_hash === finding.codeHash;
