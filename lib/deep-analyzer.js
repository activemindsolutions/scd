...
async function deepAnalyze(findings, opts = {}) {
  ...
  try {
    ...
    // Token spend for this deep run (server aggregate; absent on older servers).
    if (data.token_usage) results._tokenUsage = data.token_usage;
    return results;
...
function formatDeepSection(findings, deepResults) {
  ...
  for (const [filePath, analyses] of deepResults) {
    ...
    if (analyses[0]?._error) {
      lines.push(`\n  ${RED}✗ ${stripControl(filePath)}${RESET}`);
    ...
    for (const analysis of analyses) {
      ...
      const { color, tag } = sevConfig(sev);

      lines.push('');
      lines.push(`  ${color}${BOLD}${tag} ${analysis.ruleId} · ${stripControl(filePath)}:${analysis.line}${RESET}`);
