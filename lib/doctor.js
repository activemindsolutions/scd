...
// any warning was shown. Reset at the start of every doctor() run.
let warnCount = 0;
    ...
    if (centralUrl) {
      ...
      try {
        ...
        if (repoRoot) {
          ...
          // Re-assert approved exceptions; surface divergence the server could
          // not auto-heal (needs re-approval / conflict) as a health warning.
          try {
            const rc = await em.reassertApprovedExceptions(repoRoot);
            if (rc.healed) info(`${rc.healed} exception(s) re-applied on the server`);
            if (rc.reapproval_required) {
              warn(`${rc.reapproval_required} exception(s) need re-approval on the server (no approval record)`,
                'A team-lead must re-approve them in the dashboard.');
              allOk = false;
            }
            if (rc.conflict_rejected) {
              warn(`${rc.conflict_rejected} exception(s) conflict with a server-side rejection — manual resolve`);
              allOk = false;
            }
            // Not re-asserted because the finding is not reported to the server.
            // scope-excluded is expected (info); file-removed means a stale local
            // exception (rename/delete) worth cleaning up.
            if (rc.skippedScoped) info(`${rc.skippedScoped} exception(s) not sent — scope-excluded`);
            if (rc.skippedGone)   info(`${rc.skippedGone} exception(s) not sent — file renamed/deleted (stale local exception)`);
          } catch { /* non-fatal */ }
  ...
  try {
    ...
    if (centralUrl) {
      ...
      if (result.status === 0) {
        ...
        // Reachable, but no detailed status was returned. Prompt for a token when
        // none is configured.
        ok('scd-server reachable', baseUrl);
        if (!token) {
          console.log(`${DIM}    Configure a token for full status: scd configure --token <token>${RESET}`);
        }
      }

      // Authenticated token-validity probe. /health does NOT validate the
