...
async function doctor() {
  ...
  try {
    ...
    if (hooksPath === HOOKS_DIR) {
      ...
      console.log('\x1b[90m    This disables hooks for ALL repos on this machine.\x1b[0m');
      console.log('\x1b[90m    Fix: git config --global core.hooksPath "' + HOOKS_DIR + '"\x1b[0m');
      allOk = false;
    } else {
      warn('Global hooks pointing to unexpected directory', hooksPath);
      console.log('\x1b[90m    Expected: ' + HOOKS_DIR + '\x1b[0m');
      console.log('\x1b[90m    Fix: git config --global core.hooksPath "' + HOOKS_DIR + '"\x1b[0m');
    }
  } catch {
  ...
  try {
    ...
    if (hookStatus.status === 'disabled') {
      ...
      console.log('\x1b[90m    Re-enable with: scd repo hooks --enable\x1b[0m');
  ...
  try {
    ...
    if (centralUrl) {
      ...
      if (stale > 0) {
        ...
        console.log('\x1b[90m    Run: scd repo --verify --clean to purge stale events\x1b[0m');
        allOk = false;
      } else if (grace) {
    ...
    if (centralUrl) {
      ...
      if (result.status === 0) {
        ...
        try {
          ...
          if (srvVer) console.log(`\x1b[90m    Server version:   v${srvVer}\x1b[0m`);
          if (minVer) console.log(`\x1b[90m    Min CLI version:  v${minVer}\x1b[0m`);

          const { getVersionWarning } = require('./version-check');
          const warn = getVersionWarning();
          if (warn) {
            console.log('');
            console.log(`\x1b[33m    ${warn.replace(/\n   /g, '\n    ')}\x1b[0m`);
            allOk = false;
          }
        } catch { /* non-fatal */ }
        ...
        try {
          ...
          console.log(`\x1b[90m    Server timeout: ${fmt(sTout)}  ·  Deep timeout: ${fmt(dTout)}\x1b[0m`);
        } catch { /* non-fatal */ }
      } else {
