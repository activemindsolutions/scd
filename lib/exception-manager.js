...
async function reassertApprovedExceptions(repoRoot) {
  const result = { sent: 0, skipped: 0, healed: 0, converged: 0, pending: 0, reapproval_required: 0, conflict_rejected: 0 };
  ...
  try {
    ...
    approved = candidates.filter((e) => {
      ...
      if (isFileExcluded(scope, abs, repoRoot).excluded) return false;   // scoped-out
      if (!fs.existsSync(abs)) return false;                             // gone (rename/delete)
      return true;
    });
    result.skipped = candidates.length - approved.length;
  } catch {
    return result;
  }
