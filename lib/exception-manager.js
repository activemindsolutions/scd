...
async function addException(repoRoot, opts, type = 'exception') {
  ...
  // to deliver, status was already 'approved' locally on line 187. With central
  ...
  if (!isStandalone) {
    ...
    // (e.g. after scanning with the server down) hits a FOREIGN KEY 500
    // before the events flush creates the repo. See fix(delivery-order).
    try { await require('./push-queue').flushEvents(repoRoot); }
    catch { /* non-fatal */ }

    try {
      const result = await pushPendingExceptions(repoRoot);
      deliveredNow = result.deliveredIds.has(excId);
...
async function pushPendingExceptions(repoRoot) {
  ...
  // Resolve payload for each pending id from config.yml. Ids without a
  // matching exception record are ORPHANS — drop them silently. Otherwise
  // a manually-deleted exception would loop forever in the tracker.
  let cfgIndex;
  try {
    const { loadConfig } = require('./config');
    const cfg = loadConfig(repoRoot);
    cfgIndex = new Map((cfg.exceptions || []).map(e => [e.id, e]));
  } catch {
    cfgIndex = new Map();
  }
  ...
  try {
    ...
    // Network error / timeout / non-2xx — leave tracker intact, quiet.
    result.remaining = tracker.pendingCount(repoRoot);
...
function updateExceptionStatus(repoRoot, serverEx, status, reviewedBy, comment) {
  const configPath = require('./store').configPath(repoRoot);
  if (!fs.existsSync(configPath)) return { found: false, changed: false, error: null };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 1. finding_id — strongest, only when both sides have it.
    const isByFindingId = serverEx.finding_id
      && entryText.includes(`finding_id: "${serverEx.finding_id}"`);

    // 3/4. canonical tuple with code_hash; legacy fallback only when hashless.
    const hashField = entryText.match(/line_hash:\s*"([^"]*)"/);
    const localHash = hashField ? hashField[1] : null;
    ...
    if (isByFindingId || isById || isByRuleLine) {
      firstLine = i;
      lastLine  = j - 1;
      break;
    }
  ...
  if (firstLine === -1) {
    // No local entry matches this decision — caller treats as 'unknown'.
    return { found: false, changed: false, error: null };
  }

  // Update status field within entry — track whether it actually changed so a
  // redelivered decision (status already at target) reports as a no-op.
  let statusFound   = false;
  let statusChanged = false;
  for (let i = firstLine; i <= lastLine; i++) {
    if (/^\s+status:/.test(lines[i])) {
      statusFound = true;
      const next = lines[i].replace(/status: "[^"]*"/, `status: "${status}"`);
      if (next !== lines[i]) { lines[i] = next; statusChanged = true; }
      break;
    }
  }
  if (!statusFound) {
    // Matched entry has no status field — malformed local state. Redelivery
  ...
  // Add reviewer info + db_id if not already in this entry. db_id is the
  // server's numeric exception id, present only on the pull channel
  // (/exceptions/approved returns SELECT *); push-response records carry no
  // numeric id, so db_id is simply omitted there.
...
function applyServerDecisions(repoRoot, decisions) {
  const result = { applied: 0, alreadyApplied: 0, unknown: 0, failed: 0, records: [] };
  ...
  for (const d of list) {
    ...
    if (status !== 'approved' && status !== 'rejected') {
      rec.outcome = 'failed';
      rec.error   = 'unsupported status: ' + String(status);
      result.failed++;
...
// or unknown) the mark becomes that record's updated_at string, VERBATIM. On
// the first failed record — or a record whose updated_at is unusable — stop:
// the failing record and everything after it redeliver next flush.
//
...
module.exports = { addException, addExceptionById, syncExceptions, pushPendingExceptions, getSyncNotice, listExceptions, removeRejected, applyServerDecisions, applyFlushDecisions };
