...
async function addExceptionById(repoRoot, findingId, opts, type = 'exception') {
  ...
  if (!findingId) {
    ...
    console.error(`${DIM}   Finding IDs are shown in scd scan --verbose output (e.g. f-20eb992e)${RESET}\n`);
    process.exit(1);
  }

  if (!findingId.startsWith('f-') || findingId.length !== 10) {
    console.error(`\n${RED}❌ Invalid finding ID: ${findingId}${RESET}`);
    console.error(`${DIM}   Finding IDs look like: f-20eb992e (shown in scd scan --verbose)${RESET}\n`);
    process.exit(1);
  }
  ...
  // Delegate to addException with resolved fields
...
async function addException(repoRoot, opts, type = 'exception') {
  ...
  if (fs.existsSync(filePath)) {
    ...
    console.log(`\n\x1b[33m⚠  File not found: ${file}\x1b[0m`);
    console.log(`\x1b[90m   Exception will be created without code hash.\x1b[0m`);
    console.log(`\x1b[90m   This means it matches ANY occurrence of ${rule} in that file — not just line ${lineNum}.\x1b[0m\n`);
...
async function syncExceptions(repoRoot) {
  ...
  console.log('\n\x1b[36m↓ Syncing exceptions from scd-server…\x1b[0m');
  ...
  try {
    ...
    if (msg.includes('HTTP 503') && msg.includes('License invalid')) {
      console.error('\n\x1b[33m⚠  Server license invalid — exceptions cannot be synced.\x1b[0m');
      console.error('\x1b[90m   Contact your local scd-server administrator to resolve this.\x1b[0m\n');
    } else {
...
function getSyncNotice(repoRoot) {
  try {
    ...
    const icon  = stale ? '\x1b[33m⚠ \x1b[0m' : '\x1b[36mℹ \x1b[0m';
    ...
    return `${icon}\x1b[90m ${pending.length} exception(s) pending approval – ${age} – run \x1b[0m\x1b[1mscd sync\x1b[0m`;
  } catch {
    return null;
  }
}
...
function listExceptions(repoRoot, statusFilter = 'all') {
  ...
  if (!valid.includes(statusFilter)) {
    console.error(`\x1b[31m❌ Invalid status: ${statusFilter}. Use: pending | approved | rejected | all\x1b[0m`);
    process.exit(1);
  }
  ...
  for (const ex of list) {
    ...
    console.log(`  ${DIM}File:   ${RESET}${ex.file}${ex.line ? ':' + ex.line : ''}`);
