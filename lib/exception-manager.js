...
function writeException(repoRoot, exception) {
  const { loadExceptions, buildExceptionRecord, writeExceptions } = require('./exceptions-store');
...
function updateExceptionStatus(repoRoot, serverEx, status, reviewedBy, comment) {
  const { loadExceptions, writeExceptions } = require('./exceptions-store');
  const records = loadExceptions(repoRoot);
...
function listExceptions(repoRoot, statusFilter = 'all') {
  ...
  for (const ex of list) {
    ...
    if (ex.archived_at) console.log(`  ${DIM}Archived: ${RESET}${ex.archive_reason || 'archived'}  ${DIM}(${ex.archived_at.slice(0, 10)})${RESET}`);
...
module.exports = {
  ...
  archiveException, withdrawException, archiveResolvedExceptions, ARCHIVE_REASONS,
};
