...
async function reassertApprovedExceptions(repoRoot) {
  const result = { sent: 0, healed: 0, converged: 0, pending: 0, reapproval_required: 0, conflict_rejected: 0 };
  ...
  try {
    ...
    approved = loadExceptions(repoRoot).filter(e => e.status === 'approved' && e.line_hash);
  } catch {
    return result;
  }
