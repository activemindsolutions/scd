...
function writeException(repoRoot, exception) {
  ...
  const block = [
    ...
    `    file: "${exception.file}"`,
    `    line: ${exception.line}`,
    exception.code_hash ? `    line_hash: "${exception.code_hash}"` : null,
    `    reason: "${exception.reason}"`,
...
async function syncExceptions(repoRoot) {
  ...
  try {
    ...
    // Apply approved exceptions locally
    let applied = 0;
    let skipped = 0;
...
module.exports = { addException, syncExceptions, getSyncNotice, listExceptions, removeRejected };
