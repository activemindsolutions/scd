 ...
 *    same time): the read-modify-write cycle is serialised by `withFileLock`
 *    (#170) — the read happens inside the lock, so a concurrent writer's diff is
 *    not lost. The atomic rename still guards against corruption underneath.
 */
...
function markPending(repoRoot, exceptionId) {
  ...
  withFileLock(exceptionsPushPath(repoRoot), () => {
