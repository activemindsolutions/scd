 ...
 *    same time): same read-modify-write race as findings-store — to be solved
 *    by the same future write-lock, not addressed here. The atomic rename
 *    ensures one writer wins cleanly without corruption — at worst an entry is
 *    lost and will be re-marked on the next operation.
 */
...
function writePendingAtomic(repoRoot, records) {
  // Shared atomic writer (unique per-writer tmp + atomic rename) — same primitive
  // as findings.jsonl / exceptions.jsonl, so the tmp-collision fix applies here too.
  writeJsonlAtomic(exceptionsPushPath(repoRoot), records);
}
