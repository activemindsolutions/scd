...
function mapConfigExceptionToRecord(ex) {
  ...
  const {
    created_date,                         // renamed → created_at
    finding_id,                           // intentionally not stored (§2.1)
    id, type, tag, status, rule, file, line, line_hash, reason,
  ...
  rec.created_at = createdDateToUtcIso(created_date);
  ...
  // names (branch, archived_at, archive_reason, expires, review_date) land here
  // once their sub-tracks ship — E1a does not write them but must not choke on
  // their later presence. `finding_id` and `created_date` were destructured out
