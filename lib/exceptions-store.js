...
// #170: serialise a read-modify-write cycle on exceptions.jsonl. The read MUST
// happen inside fn, so callers wrap their whole loadExceptions→…→writeExceptions
// span. One lock path per repo (exceptionsPath), shared by every writer.
function withExceptionsLock(repoRoot, fn) {
  return withFileLock(exceptionsPath(repoRoot), fn);
}
...
function buildExceptionRecord(fields) {
  const {
    ...
    expires,                              // E1c.3 expiry / review deadline
  ...
  if (expires != null) rec.expires = expires;                      // E1c.3 — expiry deadline
...
function loadExceptionsWithBootstrap(repoRoot) {
  ...
  // Might bootstrap. Serialise + RE-CHECK under the lock (#170): two concurrent
  // first reads must not both bootstrap — the second would find config already
  // stripped by the first, map it to empty, and clobber the freshly-built store.
  // The steady-state (already-migrated) read path above stays lock-free; only the
  // one-time materialization takes the lock.
  return withExceptionsLock(repoRoot, () => {
    const cfg2 = loadConfig(repoRoot);
    if (cfg2[MIGRATION_MARKER] === true) {
      return { records: loadExceptions(repoRoot), bootstrapped: false };
    }
    const cfgEx = Array.isArray(cfg2.exceptions) ? cfg2.exceptions : [];
    if (cfgEx.length === 0) {
      return { records: loadExceptions(repoRoot), bootstrapped: false };
    }

    // Bootstrap / re-bootstrap.
    const mapped = cfgEx.map(mapConfigExceptionToRecord).filter(Boolean);
    const target = exceptionsPath(repoRoot);   // creates the store dir at 0o700
    ...
    const gate   = verifyBootstrapGate(onDisk, cfgEx);
...
module.exports = {
  ...
  // Store write API:
  withExceptionsLock,   // #170: lock a loadExceptions→…→writeExceptions cycle
  writeExceptions,
