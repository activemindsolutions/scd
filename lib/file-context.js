...
function buildFileContext(filePath, content) {
  ...
  // Definitive: strong test filename signal (.test.js, .spec.ts, test_.py, _test.go etc.)
  // These naming conventions are language-standard — no content confirmation needed.
  // Contrast with path-only signals (/tests/ directory) which remain tentative.
  if (!fileType && !tentativeType && STRONG_TEST_FILENAME_RE.test(basename)) {
    fileType = FILE_TYPES.TEST;
    signals.push(`filename: strong test pattern (${basename}) — definitive`);
  }

  // Tentative: weaker filename signals (Test.java, .test, .spec without extension)
