 ...
 *
 *   - code_hash: sha256(<raw line>).slice(0,32). Hashes the raw line ONLY;
 *     never includes filePath, line number, or any other field.
 *   - finding_id: content-based via makeFindingId in this module; line numbers
 *     are display-only and must never enter identity; never re-implement the
 *     algorithm outside this module.
 *   - A finding must survive a line shift without fragmenting its history in
 *     findings.jsonl. The reconciler trusts finding_id as the only identity;
 *     if identity changes when a line moves,
 ...
 * scanner-full.js (buildFinding) is the mechanism that makes the degenerate id
 * functionally unreachable in storage and push payloads; this module need not
 * (and must not) guard against degeneracy itself.
 *
 * ─── Migration note ───────────────────────────────────────────────────────
 *
