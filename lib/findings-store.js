 ...
 *     scanner bug; see the `codeHash` field in `buildFinding()` in scanner-full.js —
 *     the `: null` branch is a relic from the redaction era and should be removed in
 *     a follow-up.)
...
function writeFindingsAtomic(repoRoot, records) {
  ...
  writeJsonlAtomic(target, records);        // shared tmp → fsync → rename (jsonl-atomic.js)
}

function updateFindings(repoRoot, scanFindings, options = {}) {
  // #170: serialise the entire load→modify→write cycle. Two concurrent same-repo
  // scans must not each read the same baseline and clobber each other's diff —
  // the read MUST happen inside the lock, so the whole body runs under it.
  return withFileLock(findingsPath(repoRoot), () =>
    updateFindingsLocked(repoRoot, scanFindings, options));
}

function updateFindingsLocked(repoRoot, scanFindings, options = {}) {
 ...
 *
 * Decision rationale: bootstrap never passes coverage, so it never resolves —
 * a cache is not coverage proof.
 */
