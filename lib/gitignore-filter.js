...
/**
 * lib/gitignore-filter.js
 *
 * Reads and applies .gitignore rules when discovering files for scanning.
 *
 * Strategy:
...
function parseGitignoreFile(filePath) {
  ...
  for (let line of lines) {
    ...
    let regexStr = line
      ...
      .replace(/\*\*/g, '__DOUBLESTAR__')       // mark **
      .replace(/__STAR__/g, '[^/]*')            // * = anything except /
      .replace(/__DOUBLESTAR__/g, '.*')         // ** = anything
