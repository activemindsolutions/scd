...
function set(key, value) {
  // #170: serialise the read-modify-write so concurrent writers (e.g. a flush's
  // setServerVersionInfo racing a `scd configure`) don't clobber each other's
  // keys. ensureDir first so the lock file can be created in ~/.scd/. All setters
  // (setCentralUrl/Token, setServerTimeout/DeepTimeout, setScanTrace,
  // setServerVersionInfo) route through here, so locking set()/remove() covers
  // every write path.
  ensureDir();
