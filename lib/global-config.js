/**
 * global-config.js
 * Manages the global (user-level) Security Co-Pilot configuration.
 *
 * Location: ~/.security-copilot/config  (never inside a repo)
 ...
 *   - sc configure --show reveals only first 12 + last 4 chars
 */
...
function writeRaw(data) {
  ensureDir();
  const lines = [
    '# Security Co-Pilot – global configuration',
    '# Hanteras av: sc configure',
    '# Dela ALDRIG denna fil – den innehåller API-nycklar',
    '',
...
/**
 * Return a summary object for sc configure --show
 */
function showConfig() {
  const raw    = readRaw();
  const apiKey = getApiKey();
  const source = process.env.ANTHROPIC_API_KEY ? 'miljövariabel'
  ...
  return {
    ...
    apiKeySource: source || '(inte satt)',
    raw,
  };
}

module.exports = { get, set, remove, getApiKey, maskApiKey, showConfig, GLOBAL_CONFIG, GLOBAL_DIR };
