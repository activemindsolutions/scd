...
// True when a configured core.hooksPath actually points at scd's hooks dir.
// git stores the path as-is (absolute, or relative to the repo; ~ is NOT expanded
// by git), so resolve both before comparing — a foreign path (e.g. a relative
// ../other/hooks) silently bypasses scd's hooks while git still runs *something*.
function isScdHooksPath(p, repoRoot) {
  if (!p) return false;
  let resolved = p.replace(/^~(?=[/\\]|$)/, os.homedir());
  resolved = path.resolve(repoRoot || process.cwd(), resolved);
  return path.resolve(HOOKS_DIR) === resolved;
}
...
function getHookStatus(repoRoot) {
  try {
    ...
    if (localPath !== null) {
      const status = isNullDevice(localPath)            ? 'disabled'
                   : isScdHooksPath(localPath, repoRoot) ? 'enabled'
                   :                                       'shadowed';  // foreign dir → scd hooks bypassed
      return { status, hooksPath: localPath, source: 'local' };
    }
    ...
    if (globalPath !== null) {
      ...
      const status = isScdHooksPath(globalPath, repoRoot) ? 'enabled' : 'shadowed';
      return { status, hooksPath: globalPath, source: 'global' };
    }
