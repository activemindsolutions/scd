...
// ── Bygg deep-prompt (skickar ENBART statistik – ingen kod) ───────────────

function buildDeepPrompt(analysis, signals) {
  const stats = {
    period:          analysis.meta.periodDays + ' days',
    totalScans:      analysis.meta.totalScans,
    totalFindings:   analysis.meta.totalFindings,
    developers:      analysis.meta.uniqueDevelopers,
    topRules:        analysis.recurringRules.slice(0, 5).map(r => ({
                       rule: r.ruleId, count: r.count, category: r.category, severity: r.severity,
                     })),
    knowledgeGaps:   analysis.knowledgeGaps.gaps.slice(0, 4).map(g => ({
                       category: g.category, count: g.count, pct: g.percent,
                     })),
    avoidance:       { exceptRate: analysis.avoidance.exceptRate, blockRate: analysis.avoidance.blockRate },
    trend:           { signal: analysis.trend.signal, trendPct: analysis.trend.trendPct || 0 },
    timePatterns:    { lateRate: analysis.timePatterns.lateRate, peakHour: analysis.timePatterns.peakHour,
                       peakDay: analysis.timePatterns.peakWeekday },
    topHotspotFiles: analysis.fileHotspots.hotspots.slice(0, 3).map(f => ({
                       file: f.file, count: f.count, critical: f.critical,
                     })),
    devProfiles:     analysis.developers.developers.slice(0, 5).map(d => ({
                       name: d.name, findings: d.total, criticalRate: d.criticalRate,
                       topCategory: d.topCategory,
                     })),
    localSignals:    signals.map(s => ({ type: s.type, title: s.title })),
  };

  return `You are a senior application security consultant. The following is ONLY statistics (no code, no file contents) from a team's security scans.

Analyze the patterns and provide:
1. A summary (3-4 sentences) of the team's security maturity
2. The 3 most important concrete actions, prioritized by risk reduction
3. Which behavioral patterns are most concerning
4. A recommendation for what type of training would have the greatest impact

Be specific and action-oriented. No generic advice – base everything on the numbers.
Respond in English.
...
async function analyzeInsights(repoRoot, opts = {}) {
  const { deep = false, apiKey = null, days = 90 } = opts;
  ...
  // Deep-analys via Claude API
  if (deep && apiKey) {
    try {
      const prompt = buildDeepPrompt(analysis, signals);
      ...
      if (resp.ok) {
        ...
        analysis.deepInsight = data.content?.[0]?.text || null;
      } else {
        analysis.deepInsightError = `API-fel: ${resp.status}`;
      }
    } catch (err) {
      analysis.deepInsightError = err.message;
    }
  }
