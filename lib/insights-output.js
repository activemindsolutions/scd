...
'use strict';
const { RESET, BOLD, DIM, RED, GREEN, YELLOW, CYAN, BRIGHT_RED } = require('./output-constants');
...
function renderInsights(analysis) {
  if (analysis.empty) {
    console.log(`\n${DIM} ℹ  ${analysis.reason || 'No audit data found. Run scd scan to start collecting data.'}${RESET}\n`);
    return;
  }
  ...
  if (trend.signal !== 'INSUFFICIENT_DATA' && trend.weeks?.length > 2) {
    ...
    for (const w of trend.weeks.slice(-8)) {  // show at most 8 weeks
  ...
  if (fileHotspots.hotspots.length > 0) {
    ...
    for (const h of fileHotspots.hotspots) {
      ...
      const parts   = stripControl(h.file).replace(/\\/g, '/').split('/');
      const shortFile = parts.length > 2 ? '…/' + parts.slice(-2).join('/') : parts.join('/');
