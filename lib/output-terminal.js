...
function formatTerminal(findings, hookType, config = null, meta = {}) {
  if (findings.length === 0) {
    return {
      output: `\n${GREEN}${BOLD} ✅ Inga säkerhetsproblem hittades.${RESET}\n`,
      exitCode: 0,
    };
  }
  ...
  if (excepted.length > 0) {
    summaryParts.push(`${DIM}✓ ${excepted.length} undantagen${RESET}`);
  }
  ...
  lines.push('');
  lines.push(`${BOLD}┌─ Sammanfattning ${'─'.repeat(43)}┐${RESET}`);
  lines.push(`${BOLD}│${RESET}  ${summaryParts.join('   ')}`);

  // Hook/mode label on same header block
  const modeLabel = hookType === 'manual'  ? 'Manuell scanning'
                  : hookType === 'pre-commit' ? 'Pre-commit (secrets)'
                  : 'Pre-push (OWASP)';
  const skippedTotal = skipped.length + timedOut.length;
  const skippedNote = skippedTotal > 0 ? `  ${YELLOW}· ${skippedTotal} fil(er) hoppades över/timeout${RESET}` : '';
  lines.push(`${BOLD}│${RESET}  ${DIM}${modeLabel}  ·  ${unique.length} findings totalt${RESET}${skippedNote}`);
  lines.push(`${BOLD}└${'─'.repeat(60)}┘${RESET}`);
  lines.push('');

  // Skipped + timed-out files – shown just below header
  if (skipped.length > 0 || timedOut.length > 0) {
    lines.push(`${DIM}Hoppade över (visas ej i scanning):${RESET}`);
    ...
    for (const t of timedOut) {
      lines.push(`  ${YELLOW}⏱ ${fileLink(t.filePath)}  ${t.sizeKb} KB  ${RESET}${DIM}– scanning tog >30s och avbröts. Prova --no-limit med mer tid, eller exkludera filen.${RESET}`);
    }
    lines.push('');
  }
  ...
  if (Object.keys(byFile).length > 0) {
    ...
    for (const [filePath, filefindings] of Object.entries(byFile)) {
      ...
      for (const f of filefindings) {
        ...
        const warnNote = isWarn ? ` ${DIM}(varning)${RESET}` : '';
  ...
  if (shouldBlock) {
    if (hookType === 'manual') {
      lines.push(`${RED}${BOLD} ⚠️  Kritiska sårbarheter hittades – åtgärda innan push.${RESET}`);
      lines.push(`${DIM}    Manuell scanning blockerar inte. Git push kommer att blockeras.${RESET}`);
      ...
      lines.push(`${RED}${BOLD} 🚫 Push blockerad – kritiska sårbarheter måste åtgärdas.${RESET}`);
      lines.push(`${DIM}    Kör git push igen efter att du åtgärdat ovanstående.${RESET}`);
    }
  } else {
    if (hookType === 'manual') {
  ...
  if (allDetailed.length > 0) {
    ...
    for (const [ruleId, ruleFindings] of sortedRules) {
      ...
      const warnNote = isWarn ? ` ${DIM}(varning – blockerar ej)${RESET}` : '';
      ...
      if (rep.severity === 'EXPOSURE' && rep.checklist) {
        ...
        for (const f of ruleFindings) {
          lines.push(`  ${DIM}→ sc resolve --rule ${f.ruleId} --file ${f.filePath} --line ${f.line}${RESET}`);
        }
      }
    }
  }
