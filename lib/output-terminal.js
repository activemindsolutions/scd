...
function formatTerminal(findings, hookType, config = null, meta = {}) {
  ...
  if (verbose) {
    ...
    if (excepted.length > 0) {
      ...
      const notableExc = excepted.filter(f => f.severity === 'CRITICAL' || f.severity === 'HIGH');
      ...
      // Show CRITICAL and HIGH excepted findings explicitly so they are not invisible
      if (notableExc.length > 0) {
        lines.push(`  ${DIM}  Excepted findings not included above:${RESET}`);
        for (const f of notableExc) {
          const sevIcon = f.severity === 'CRITICAL' ? `${RED}🔴${RESET}` : `${YELLOW}🟠${RESET}`;
          const excType = f.exception?.type ? `  ${DIM}[${f.exception.type}]${RESET}` : '';
          const fp      = f.filePath || f.file || '?';
          const fileLine = f.line ? `${fp}:${f.line}` : fp;
          lines.push(`  ${DIM}   ${sevIcon}  ${f.ruleId}  ${fileLine}${excType}${RESET}`);
        }
      }
      lines.push('');
    }
  ...
  // ── Block/pass status ─────────────────────────────────────────────────────
  const hasActiveBlock   = active.some(f => f.blocks);
  const hasExpiredExc    = excepted.some(f => f.exception_expired);

  if (hasActiveBlock) {
    // Determine the highest blocking severity for a precise message
    const hasCritical = active.some(f => f.blocks && f.severity === 'CRITICAL');
    const hasHigh     = active.some(f => f.blocks && f.severity === 'HIGH');
    const topSev      = hasCritical ? 'CRITICAL' : hasHigh ? 'HIGH' : 'blocking';

    if (hookType === 'manual') {
      lines.push(`${RED}${BOLD} ⚠️  ${topSev === 'CRITICAL' ? 'Critical' : topSev === 'HIGH' ? 'High-severity' : 'Blocking'} vulnerabilities found – fix before pushing.${RESET}`);
      ...
      lines.push(`${RED}${BOLD} 🚫 Push blocked – ${topSev === 'CRITICAL' ? 'critical' : 'high-severity'} vulnerabilities must be fixed.${RESET}`);
      lines.push(`${DIM}    Run git push again after fixing the above.${RESET}`);
    }
  } else if (hasExpiredExc) {
    lines.push(`${YELLOW}${BOLD} ⚠️  Exception(s) expired – re-approval required before next push.${RESET}`);
    lines.push(`${DIM}    Run scd sync to check status, or contact your team lead.${RESET}`);
  } else {
    if (hookType === 'manual') {
  ...
  return {
    output: lines.join('\n'),
    exitCode: (hasActiveBlock || hasExpiredExc) ? 1 : 0,
  };
}
