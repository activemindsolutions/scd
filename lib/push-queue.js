 ...
 * scd doctor reports stale count; scd store --verify --clean can purge.
 *
...
async function flush(centralUrl, opts = {}) {
  ...
  try {
    ...
    const body  = JSON.stringify({ events: pending.map(e => e.event) });

    const response = await httpPost(http, url, body);
...
function httpPost(http, urlStr, body) {
