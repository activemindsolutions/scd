...
const FLUSH_STATUS_RANK = { rejected: 6, auth_failed: 5, license_invalid: 4, error: 3, unreachable: 2, sent: 1, empty: 0 };
 ...
 * 'rejected' (permanent client error — batch dropped, not retried).
...
async function flush(centralUrl, opts = {}) {
  ...
  for (const [repoId, groupEntries] of groups) {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      // Permanent client error — the batch will NEVER be accepted as-is (e.g. 413
      // findings-per-batch cap, 400 malformed, 422 unprocessable). Retrying wastes
      // ten flushes before it churns to stale, so drop the entries and surface it
      // loudly instead. (Not 408/429 — those are transient and fall through to the
      // attempt-bump retry path below.)
      groupEntries.forEach(e => rejectIds.add(e.id));
      status = worseFlushStatus(status, groupEmpty ? 'empty' : 'rejected');
    } else {
  ...
  for (const e of entries) {
    ...
    if (rejectIds.has(e.id))  continue;   // permanent client error — drop, do not retry
...
/**
 * Human-facing classification of a flush() status for the EXPLICIT commands
 * (scd sync / scd doctor). A silent best-effort push must not swallow a real
 * server-side failure — a rejected batch, a bad token, or an invalid licence
 * means the user THINKS their data synced when it did not. Returns null for the
 * quiet outcomes ('sent'/'empty'/null/'unreachable'-during-scan is caller's call).
 *
 * @returns {null | { level: 'warn'|'info', message: string, hint: (string|null) }}
 */
function flushStatusNotice(status) {
  switch (status) {
    case 'rejected':
      return { level: 'warn', message: 'scd-server rejected the batch as invalid or too large — those events were dropped',
               hint: 'A very large scan can exceed the server ingest limit; re-scan a narrower scope if findings are missing.' };
    case 'error':
      return { level: 'warn', message: 'scd-server rejected the sync (server error)',
               hint: 'Events are kept and retried — inspect with: scd queue list' };
    case 'auth_failed':
      return { level: 'warn', message: 'scd-server rejected the token — event delivery is blocked',
               hint: 'Set a valid token: scd configure --token <token>' };
    case 'license_invalid':
      return { level: 'warn', message: 'scd-server licence is invalid — events held in the queue',
               hint: 'Delivery resumes automatically once the licence is restored.' };
    case 'unreachable':
      return { level: 'info', message: 'scd-server unreachable — events queued for later', hint: null };
    default:
      return null;   // 'sent' | 'empty' | null → nothing to surface
  }
}
