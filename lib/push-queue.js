...
async function flush(centralUrl, opts = {}) {
  ...
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      if (opts.verbose) {
        console.log(`\x1b[90m  ↑ Central unavailable (503) – ${pending.length} event(s) held in queue\x1b[0m`);
      }
      return 'unreachable';
    } else {
    ...
    if (opts.verbose) {
      console.log(`\x1b[90m  ↑ Central unreachable – ${pending.length} event(s) held in queue\x1b[0m`);
    }
    return 'unreachable';
  }
}
