 ...
 * scd doctor reports stale count; scd list verify --clean can purge.
 * TODO: scd list verify --clean is currently per-repo but the push queue
...
async function flush(centralUrl, opts = {}) {
  ...
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      // Cache server version info + apply any server decisions piggybacked on
      // the events/batch response (the sync_exceptions push channel).
      try {
        ...
        // Push-response delivery channel. The `sync_exceptions` KEY being
        // present (an array, even empty) is positive evidence the channel is
        // live and nothing is pending — that, not a bare 2xx, is what refreshes
        // the staleness timestamp. An old server omits the key entirely, so the
        // stale-sync nag correctly survives (no status change without evidence).
        if (opts.repoRoot && Array.isArray(parsed.sync_exceptions)) {
          if (parsed.sync_exceptions.length > 0) {
            // applyFlushDecisions is idempotent, silent on no-op, never throws.
            require('./exception-manager').applyFlushDecisions(opts.repoRoot, parsed.sync_exceptions);
          }
          try { require('./store').updateLastSynced(opts.repoRoot, []); }
          catch { /* non-fatal */ }
        }
      } catch { /* non-fatal */ }
...
function listEntries() {
  return readQueue().map(e => {
    ...
    return { ...e, stale: reason !== null, reason };
  });
}
...
function resetAttempts() {
  ...
  const cleared   = entries.map(e => ({ ...e, attempts: 0, lastAttempt: null }));
  writeQueue(cleared);
