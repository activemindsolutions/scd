...
function escHtml(str) {
  return String(str || '')
    // Strip terminal control sequences (ESC etc.) from untrusted content so a report
    // opened in a terminal pager/preview cannot be used to drive the terminal. Keep \t \n.
...
function renderFindingsDetail(findings, repoRoot = '') {
  ...
  return Object.entries(byFile).sort(([a], [b]) => a.localeCompare(b)).map(([filePath, items]) => {
    ...
    // encodeURI the attacker-controlled path so a crafted file name cannot alter the
    // vscode:// URI structure; escHtml on the attribute then handles HTML context.
    const vsodeUrl = `vscode://file/${encodeURI(absPath)}`;

    const findingRows = severities.flatMap(sev =>
      (bySeverity[sev] || []).map(f => {
        const vscodeLine = `vscode://file/${encodeURI(absPath)}:${f.line}`;
...
function buildCSS() {
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .brand { display: flex; align-items: baseline; gap: 0.6rem; margin-bottom: 1.5rem; }
  .brand-mark {
  ...
  .brand-mark .brace { color: var(--muted); font-weight: 400; }
  .brand-name {
    font-size: 0.95rem;
    font-weight: 600;
    letter-spacing: 0.01em;
  ...
  .report-subject { display: flex; align-items: baseline; gap: 0.75rem; flex-wrap: wrap; }
  .subject-label {
    font-size: 0.68rem;
    font-weight: 700;
  ...
  .report-repo { font-size: 1.6rem; font-weight: 700; line-height: 1.2; }
  .report-target { font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace; color: var(--muted); font-size: 0.85rem; margin-top: 0.5rem; }
  ...
  .sev-count { font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace; font-size: 0.85rem; font-weight: 700; width: 30px; text-align: right; }
  ...
  .owasp-id { font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace; font-size: 0.7rem; font-weight: 700; color: var(--muted); display: block; }
  ...
  .stat-value { font-size: 2rem; font-weight: 800; line-height: 1; font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace; }
  ...
  .count-cell { font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace; font-weight: 700; text-align: center; }
  .file-list code { font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace; font-size: 0.75rem; color: var(--muted); background: var(--surface2); padding: 0.1rem 0.35rem; border-radius: 3px; margin-right: 0.25rem; }
  ...
  .deep-file-header { font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace; font-size: 0.82rem;
  ...
  .deep-code { background: var(--bg); border: 1px solid var(--border); border-radius: 6px;
               padding: 0.75rem 1rem; font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace;
  ...
  .deep-item-file { font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace; font-size: 0.78rem;
  ...
  .deep-orig-rule { font-family: 'JetBrains Mono', ui-monospace, Menlo, Consolas, monospace; font-size: 0.78rem;
                    color: var(--muted); }
...
<div class="page">
  ...
  <header class="report-header">
    <div>
      <div class="brand">
        <span class="brand-mark"><span class="brace">{</span>scd<span class="brace">}</span></span>
        <span class="brand-name">Secure Code by Design</span>
      </div>
      <div class="report-subject">
        <span class="subject-label">Security report</span>
        <h1 class="report-repo">${escHtml(displayName)}</h1>
      </div>
