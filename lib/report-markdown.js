...
function generateMarkdown(findings, opts = {}) {
  ...
  // ── Header ────────────────────────────────────────────────────────────────
  lines.push(`# 🛡️ Security Co-Pilot – Säkerhetsrapport`);
  lines.push('');
  ...
  lines.push(`| Totalt findings | **${findings.length}** |`);
  ...
  lines.push('|---|---|---|');
  const sevDesc = {
    CRITICAL: 'Direkt exploaterbar – åtgärda omedelbart',
    HIGH:     'Hög risk – åtgärda inom sprint',
  ...
  lines.push('');
  lines.push('Kryssa av när varje finding är åtgärdad:');
  lines.push('');
  ...
  lines.push('---');
  lines.push('');
  lines.push(`_Rapport genererad av **Security Co-Pilot** · ${dateStr}_`);
  lines.push('');
