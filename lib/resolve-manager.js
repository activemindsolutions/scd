 ...
 *
 * Usage: security-copilot resolve --rule FRONT-001 --file src/maps/config.js --line 3
 *
 * Creates a resolution record in .securityagent.yml under `resolutions:`
...
async function resolveExposure(repoRoot, opts) {
  ...
  if (!rule || !file || !line) {
    console.log(`\n\x1b[31mAnvändning: security-copilot resolve --rule <id> --file <path> --line <n>\x1b[0m`);
    console.log(`${DIM}Exempel: security-copilot resolve --rule FRONT-001 --file src/maps/config.js --line 3${RESET}\n`);
    process.exit(1);
  }
  ...
  console.log(`\n${CYAN}${BOLD}Security Copilot – Resolve EXPOSURE finding${RESET}`);
  ...
  if (ruleInfo?.checklist) {
    console.log(`\n${BOLD}Bekräfta att följande är på plats:${RESET}`);
  ...
  const action_taken = await ask('Åtgärd vidtagen (beskriv vad som gjorts):   ');
  const resolved_by  = await ask('Hanterat av (e-post):                        ');
  const reviewDays   = await ask('Granskas om (dagar, Enter = 180):            ');
  rl.close();
  ...
  console.log(`\n${GREEN}${BOLD}✅ Åtgärd registrerad (${resId})${RESET}`);
  console.log(`${DIM}   Granskas: ${review_date} (om ${days} dagar)${RESET}`);
  console.log(`${DIM}   Finding visas nu som "Hanterad" i rapporter tills granskning krävs.${RESET}\n`);
}
