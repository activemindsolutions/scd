/**
 * rule-registry.js
 * Central catalogue of all Security Co-Pilot rules.
 *
 * Normalises every rule into a consistent shape:
 *   { id, name, severity, category, languages, matchMode, why, scenario, fix }
 *
 * Used by: sc rules (list/search/detail)
 *          sc report (rule metadata in reports)
 *          sc insights (category grouping)
 */

'use strict';

// Rules bundle version – bump on rule additions/fixes (minor) or ID changes (major)
// Independent of CLI version: sc 0.2.0 can ship with rules 1.3.0
...
const { ALL_RULES: PY_RULES, ALL_EXPOSURE_RULES: PY_EXPOSURE }   = require('./rules/rules-python');
const { ALL_RULES: PHP_RULES, ALL_EXPOSURE_RULES: PHP_EXPOSURE } = require('./rules/rules-php');
const { ALL_RULES: ASPX_RULES }                                   = require('./rules/rules-aspx');
const { ALL_RULES: ASPX_CS_RULES }                                = require('./rules/rules-aspx-cs');
const { ALL_RULES: SF_RULES,
        CONTENT_RULES: SF_CONTENT,
        FILENAME_RULES: SF_FILENAME }                             = require('./rules/rules-sensitive-files');
const { ALL_INFRA_RULES }                                         = require('./rules/rules-infra-leakage');
