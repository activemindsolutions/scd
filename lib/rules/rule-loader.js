 ...
 *
 * Available conditions (evaluated in order within each rule object):
 *   if_file_context   {string|string[]}  Matches classifyFileContext() result.
 *                     Values: "frontend" | "backend" | "config" | "doc" | "test"
 *   if_path_contains  {string[]}         Matches if filePath contains any term (case-insensitive)
 *   if_line_contains  {string[]}         Matches if lineRaw contains any term (case-insensitive)
 *   default           {string}           Fallback — always matches
 *
 * Example:
 *   "confidence_rules": [
 *     { "if_file_context": ["test", "backend", "doc"], "then": "LOW"  },
 *     { "if_file_context": "frontend",                  "then": "HIGH" },
 *     { "if_line_contains": ["nonce", "csrf", "oauth"], "then": "HIGH" },
