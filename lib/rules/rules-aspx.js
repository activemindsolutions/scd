/**
 * rules-aspx.js
 * Security rules for ASP.NET Web Forms (.aspx, .ascx, .master, Web.config).
 * Based on real-world findings from classic ASP.NET (4.x) codebases.
 * Covers: ASPX markup patterns, Web.config misconfigurations.
 */

// ── Diagnostic / Debug exposure ───────────────────────────────────────────────
...
const DIAG_RULES = [
  {
    ...
    // Matches @Page directive on files named Diagnostics/Debug/Admin etc.
    // or pages that render connection strings, data paths, ODBC drivers
    pattern: /<%@\s*Page[^%]*%>[\s\S]{0,2000}(?:Connection\s*string\s*(?:value|position)|ODBC|lblDBPath|lblDataPath|lblSystem|lblUnmanaged)/gi,
  ...
  },
  {
    ...
    severity: 'CRITICAL',
    category: 'Security Misconfiguration (OWASP A05)',
    // Matches HyperLink or anchor pointing to .txt/.log files
    pattern: /(?:NavigateUrl|href)\s*=\s*["'~\/]*[^"']{0,60}(?:_Error|_Debug|error|debug|log|trace)\s*\.(?:txt|log)["'\s]/gi,
  ...
  },
  {
    ...
    severity: 'MEDIUM',
    category: 'Security Misconfiguration (OWASP A05)',
    // Matches server controls with Debug/Version/Diag in their ID
    pattern: /(?:asp:Label|asp:Literal)[^>]*ID\s*=\s*["'](?:lbl)?(?:Debug|debug|Version|Diag|diag|BuildInfo)[^"']*["']/gi,
...
const XSS_RULES = [
  {
    ...
    // Matches <% Response.Write(...) %> or <%Response.Write(...)%>
    // Does NOT match <%: ... %> which is the HTML-encoded shorthand
    pattern: /<%\s*Response\.Write\s*\([^%]{1,200}%>/gi,
  ...
  },
  {
    ...
    severity: 'HIGH',
    category: 'Injection (OWASP A03)',
    // Matches <%= expr %> — raw unencoded output
    // Excludes: <%: (encoded), ResolveUrl (path helper), ClientScript, safe helpers
    pattern: /<%=\s*(?!(?:ResolveUrl|ClientScript|ScriptManager|GetRouteUrl|Url\.Content))[A-Za-z_$][^%]{1,150}%>/g,
  ...
  },
  {
    ...
    severity: 'HIGH',
    category: 'Injection (OWASP A03)',
    pattern: /<%[=:]?\s*Request\s*\.\s*(?:QueryString|Form|Params|Item)\s*\[/gi,
    antipattern: /Server\.HtmlEncode|HttpUtility\.HtmlEncode|AntiXss\./i,
    lookahead: 200,
...
// ── Page directive misconfigurations ──────────────────────────────────────────
...
const CONFIG_RULES = [
  {
    ...
    severity: 'HIGH',
    category: 'Security Misconfiguration (OWASP A05)',
    pattern: /EnableEventValidation\s*=\s*["']False["']/gi,
  ...
  },
  {
    ...
    severity: 'HIGH',
    category: 'Security Misconfiguration (OWASP A05)',
    pattern: /<%@\s*Page[^%]*\bDebug\s*=\s*["']true["']/gi,
...
const WEBCONFIG_RULES = [
  ...
  },
  {
    ...
    severity: 'HIGH',
    category: 'Security Misconfiguration (OWASP A05)',
    // Matches trace enabled="true" or localOnly="false"
    pattern: /<trace\s[^>]*(?:enabled\s*=\s*["']true["'][^>]*localOnly\s*=\s*["']false["']|localOnly\s*=\s*["']false["'][^>]*enabled\s*=\s*["']true["'])/gi,
  ...
  },
  {
    ...
    severity: 'HIGH',
    category: 'Security Misconfiguration (OWASP A05)',
    pattern: /<compilation[^>]*\bdebug\s*=\s*["']true["']/gi,
    ...
    scenario: 'Combined with customErrors Off (also present in this codebase), debug=true means every unhandled exception shows full source code, variable values, and server paths. The error log in this case confirmed connection strings appearing in error output.',
  ...
  },
  {
    ...
    pattern: /<add\s[^>]*(?:connectionString|ConnectionString)\s*=\s*["'][^"']{0,200}(?:Password|PWD|pwd)\s*=[^"']{1,50}["']/gi,
    antipattern: /configProtectedData|EncryptedData|encrypt/i,
    lookahead: 300,
  ...
  },
  {
    ...
    severity: 'HIGH',
    category: 'Security Misconfiguration (OWASP A05)',
    pattern: /enableViewStateMac\s*=\s*["']false["']|ViewStateEncryptionMode\s*=\s*["']Never["']/gi,
...
const REDIRECT_RULES = [
  {
    ...
    severity: 'MEDIUM',
    category: 'Security Misconfiguration (OWASP A05)',
    // Matches URL construction with backurl, returnUrl, returnto, redirect etc. from current location
    pattern: /[?&"'](backurl|returnUrl|returnto|redirect|next|goto)\s*["']?\s*[+=]\s*(?:window\.location|Request\.|location\.href)/gi,
...
const LIBRARY_RULES = [
  {
    ...
    // Matches jquery-1.x.y or jquery-2.x.y in script src
    pattern: /jquery[-_](?:1\.\d+\.\d+|2\.\d+\.\d+)(?:\.min)?\.js/gi,
    ...
    fileTypes: ['aspx', 'ascx', 'master', 'html'],
  },
];

// ── Sensitive file exposure (log files, config backups in web root) ────────────

const FILE_EXPOSURE_RULES = [
  {
    id: 'ASPX-FILE-001',
    name: 'Sensitive file in web root — log or debug file publicly accessible',
    severity: 'CRITICAL',
    category: 'Sensitive Data Exposure (OWASP A02)',
    // Matches .txt/.log files whose names contain log/error/debug/trace patterns
    // Used by scanner-manual in deployment audit mode
    pattern: /^[\w\-. ]*(?:error|debug|log|trace|diag)[\w\-. ]*\.(?:txt|log)$/i,
    ...
    fileTypes: ['txt', 'log'],
    matchMode: 'filename', // signal to scanner to match on filename, not content
  },
];
