...
const LOCALHOST_RULES = [
  {
    ...
    antipattern: /(?:example|sample|placeholder|TODO|FIXME|demo|mock|fake|dummy|test|spec|getenv|process\.env|os\.environ|System\.getenv|\$_ENV|\$_SERVER|\)\s*[?:?]{1,2}\s*['"].*localhost|\|\|\s*['"].*localhost|,\s*['"]localhost['"]|,\s*default\s*=\s*['"]localhost|fallback)/i,
  ...
  },
  {
    ...
    pattern: /(?:=\s*["'`]|["'`])[^\n]{0,30}::1(?::\d{2,5})?["'`]/gim,
    antipattern: /(?:e\.g\.|i\.e\.|must include|not just|example|documentation|note:|see also)/i,
    lookahead: 150,
...
const RFC1918_RULES = [
  ...
  },
  {
    ...
    pattern: /["'`\s(=:/,]192\.168\.(?:25[0-5]|2[0-4]\d|1?\d{1,2})\.(?:25[0-5]|2[0-4]\d|1?\d{1,2})(?::\d{2,5})?(?:["'`\s)/,]|$)/gim,
    antipattern: DEV_CONTEXT,
    lookahead: 150,
...
const INTERNAL_HOSTNAME_RULES = [
  ...
  },
  {
    ...
    antipattern: /(?:localhost|127\.0\.0|example|sample|placeholder)/i,
    lookahead: 100,
