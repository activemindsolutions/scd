...
// Frontend file extensions — localhost in these files is a real exposure risk
// since the code is delivered to and executed in the browser.
const FRONTEND_EXTS = new Set(['js', 'ts', 'jsx', 'tsx', 'mjs', 'cjs', 'vue', 'svelte', 'html', 'htm']);

// Documentation/log extensions — may contain localhost as examples, lower risk
const DOC_EXTS = new Set(['md', 'txt', 'log', 'rst', 'adoc']);

/**
 * Classify file context for INFRA rules.
 * Returns: 'frontend' | 'backend' | 'config' | 'doc' | 'test'
 */
function classifyFileContext(filePath) {
  ...
  // Test files — always LOW confidence regardless of extension
  ...
  if (['json', 'yml', 'yaml', 'toml', 'ini', 'env', 'conf', 'cfg', 'properties', 'xml'].includes(ext)) return 'config';
...
const LOCALHOST_RULES = [
  {
    ...
    // Confidence based on file context and match content:
    // HIGH:   frontend files (js/ts/html/vue) — localhost in browser-delivered code is a real exposure
    // MEDIUM: config files — may be legitimate defaults
    // LOW:    backend, test, doc files — localhost is expected and appropriate
    //         Also LOW: localhost embedded in natural language text (i18n example strings)
    confidence: (match, lineRaw, filePath) => {
      const ctx = classifyFileContext(filePath);
      if (ctx === 'test')    return 'LOW';
      if (ctx === 'backend') return 'LOW';
      if (ctx === 'doc')     return 'LOW';

      // LOW: localhost appears inside natural language text.
      // Patterns: word/( before localhost, or localhost(:port) followed by space+word
      // e.g. "usualmente localhost", "usualmente(localhost)", "im Format localhost:3306 im"
      // Not matched: 'tcp://localhost:5555', "host": "localhost", http://localhost:3000
      const line = lineRaw || '';
      if (/\w[\s(]localhost|\blocalhost(?::\d+)?\s+\w/.test(line)) return 'LOW';

      if (ctx === 'frontend') return 'HIGH';
      return 'MEDIUM'; // config files with direct localhost assignment
    },

    why: 'Hardcoded localhost references in frontend code indicate development-environment configuration that may have leaked into production builds. In browser-delivered code, localhost URLs always fail for end users and reveal internal service topology.',
  ...
  },
  {
    ...
    lookahead: 120,
    excludeFileTypes: ['md', 'txt', 'yml', 'yaml'],
...
const INTERNAL_HOSTNAME_RULES = [
  {
    ...
    // Confidence based on file context — same logic as INFRA-001
    confidence: (match, lineRaw, filePath) => {
      const ctx = classifyFileContext(filePath);
      if (ctx === 'test')     return 'LOW';
      if (ctx === 'backend')  return 'LOW';
      if (ctx === 'frontend') return 'HIGH';
      if (ctx === 'doc')      return 'MEDIUM';
      return 'MEDIUM';
    },
  },
  {
