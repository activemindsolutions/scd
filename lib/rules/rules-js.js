...
const CRYPTO_EXTRA_RULES = [
  {
    ...
    // Dynamic confidence based on surrounding context.
    // HIGH:   security-critical keywords (state, nonce, csrf, auth, oidc, oauth, session)
    // MEDIUM: generic security keywords (token, secret, password, key)
    // LOW:    likely UI/DOM use — id, numId, element suffix, dashboard, modal, container
    confidence: (match, lineRaw, filePath) => {
      const ctx = (lineRaw || '') + (filePath || '');

      // LOW: DOM element ID patterns — numeric suffix, UI component names
      if (/(?:numId|cssName|container|dashboard|modal|widget|panel|grid|element)\s*[+=]/.test(lineRaw)) return 'LOW';
      if (/[+]\s*Math\.random|Math\.random[^;]{0,40}(?:Id|id)\s*[=;,)]/.test(lineRaw) &&
          !/(?:nonce|state|csrf|auth|oidc|oauth|session|reset|verify)/.test(lineRaw)) return 'LOW';

      // HIGH: authentication/authorisation context
      if (/(?:nonce|state|csrf|oidc|oauth|auth|reset_?token|verify|session_?id|refresh_?token)/i.test(ctx)) return 'HIGH';

      // HIGH: security-sensitive file path
      if (/(?:auth|login|oauth|oidc|token|security|crypto)/i.test(filePath || '')) return 'HIGH';
