...
const INJECTION_RULES = [
  ...
  },
  {
    ...
    severity: 'CRITICAL',
    category: 'Injection (OWASP A03)',
    taintAware: true,  // scanner checks taint register before flagging
    ...
    pattern: /\$(?:SQL|sql|query|qry|str)\s*(?:\.?=)\s*["'][^\n"']{0,200}(?:WHERE|FROM|INTO|SET|VALUES)[^\n"']{0,100}["']\s*\.\s*\$(?!_GET|_POST|_REQUEST|_COOKIE|_SESSION|stmt|pdo|conn|db)[a-zA-Z_]\w{0,30}\s*[;,"')/]/g,
  ...
  {
    // PHP-INJ-002d: taintAware — catches interpolation of tainted local variable in SQL string
    // e.g.: $username = $_POST['username']; $query = "SELECT ... WHERE username = '$username'"
    // Pattern matches the full double-quoted SQL string containing a $var (not superglobal)
    id: 'PHP-INJ-002',
    name: 'SQL Injection – tainted variable interpolated in SQL string',
    severity: 'CRITICAL',
    category: 'Injection (OWASP A03)',
    taintAware: true,
    taintExtract: 'interpolation',
    // [^\n"]{0,300} — excludes newlines so the pattern cannot span multiple lines
    pattern: /"[^\n"]{0,300}\$(?!_GET|_POST|_REQUEST|_COOKIE|_SESSION|stmt|pdo|conn|db)[a-zA-Z_]\w*[^\n"]{0,100}"/gi,
    antipattern: /sqlsrv_query|mysqli_query|\$(?:stmt|pdo)->(?:prepare|execute)|^\s*(?:\/\/|#|\*)/i,
    lookahead: 50,
    why: 'A variable assigned from user input ($_GET/$_POST) is interpolated directly into a SQL string.',
  ...
  {
    // PHP-INJ-004b: taintAware variant — catches command injection with local tainted variable
    // e.g.: $file = $_GET['filename']; system("unzip " . $file);
    id: 'PHP-INJ-004',
    name: 'Command Injection – tainted variable in shell command',
    severity: 'CRITICAL',
    category: 'Injection (OWASP A03)',
    taintAware: true,
    taintExtract: 'func_concat',   // hint to scanner: use func/concat extraction
    pattern: /(?:shell_exec|system|exec|passthru|popen)\s*\([^)]*\.\s*\$(?!_GET|_POST|_REQUEST|_COOKIE|_SESSION|stmt|pdo|conn|db)[a-zA-Z_]\w*\s*[;)]/g,
...
const AUTH_RULES = [
  ...
  {
    // PHP-AUTH-002b: taintAware variant — catches LFI with local tainted variable
    // e.g.: $page = $_GET['page']; include($page . '.php');
    id: 'PHP-AUTH-002',
    name: 'File inclusion – tainted variable in include/require (LFI)',
    severity: 'CRITICAL',
    category: 'Broken Access Control (OWASP A01)',
    taintAware: true,
    taintExtract: 'func_concat',   // hint: first arg or concat extraction
    pattern: /(?:include|require|include_once|require_once)\s*\(?\s*\$(?!_GET|_POST|_REQUEST|_COOKIE|_SESSION)([a-zA-Z_]\w*)\s*[.;)]/g,
...
const EXPOSURE_RULES = [
  {
    ...
    scenario: 'ini_set("display_errors", 1) left in from development. A malformed request triggers a database error: "SQLSTATE[42S02]: Table \'prod_db.users_v3\' doesn\'t exist" with the full file path. Attacker now knows the database name, table structure, and internal paths.',
  ...
  },
  {
    ...
    scenario: 'APP_KEY hardcoded in config/app.php, committed to git. Attacker decrypts all user session cookies, reads encrypted fields in the database, and forges remember_me tokens for any user ID.',
  ...
  },
  {
    ...
    lookahead: 40,
    why: 'WordPress stores database credentials and cryptographic salts directly in wp-config.php. If this file is accidentally exposed via a misconfigured server or included in a git repo, all credentials and session signing keys are compromised.',
    scenario: 'wp-config.php committed to a public GitHub repo. Attacker extracts DB_PASSWORD, connects directly to the MySQL database, dumps all user password hashes and personal data, and uses AUTH_KEY to forge WordPress authentication cookies.',
