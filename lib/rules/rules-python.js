...
const INJECTION_RULES = [
  ...
  {
    id: 'PY-INJ-002',
    name: 'Command Injection – os.system / subprocess med user input',
    severity: 'CRITICAL',
    category: 'Injection (OWASP A03)',
    pattern: /(?:os\.system|os\.popen|subprocess\.(?:call|run|Popen|check_output))\s*\(\s*(?:[f]['"]|(?:cmd|command|input|user_input|request\.|args\.|kwargs\.))/g,
    why: 'Shell commands are built with user input and executed directly on the server.',
    scenario: 'An attacker sends "; cat /etc/passwd" as input and the server executes it as a shell command.',
    fix: 'Use subprocess with a list instead of a string: subprocess.run(["ls", path], shell=False). Always validate input against a whitelist.',
    fileTypes: ['py'],
  },
  {
    id: 'PY-INJ-003',
...
const PATH_RULES = [
  {
    ...
    severity: 'CRITICAL',
    category: 'Broken Access Control (OWASP A01)',
    pattern: /open\s*\(\s*(?:(?:request\.(?:args|form|GET|POST|values|files)\s*(?:\.get\s*\([^)]+\)|\[[^\]]+\])|(?:filename|file_name|filepath|file_path|fname|name|path)\s*(?=[,)]))|f["'][^"']*\{)/g,
    why: 'Using user-supplied input as a file path without sanitization allows attackers to read arbitrary files on the server using traversal sequences like ../../etc/passwd.',
