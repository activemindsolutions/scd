...
const EXPOSURE_RULES = [
  {
    ...
    lookahead: 40,
    why: 'DEBUG=True enables the Django debug page which displays full stack traces, local variable values, SQL queries, and loaded settings — including SECRET_KEY and database credentials — to anyone who triggers an error. It also disables security checks and caching.',
    scenario: 'settings.py shipped with DEBUG=True to production. Any unhandled exception returns a full debug page showing the database connection string, all installed middleware, and the complete request/response cycle to the end user.',
