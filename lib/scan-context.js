'use strict';
const { RESET, BOLD, DIM, YELLOW, CYAN } = require('./output-constants');
...
async function checkRepoOverlap(repoRoot, { interactive = true, repoId = null } = {}) {
  ...
  if (!interactive) {
    // Hook mode — can't prompt, warn and proceed
    console.log(`${YELLOW}  Continuing scan (hook mode — cannot prompt).${RESET}\n`);
