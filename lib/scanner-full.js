...
function buildFinding(rule, filePath, lineNumber, snippet, lineRaw, config, taintInfo = null, matchObj = null) {
  ...
  const finding = {
    ...
    // the redaction era; tracked in PROGRESS.md (post-pilot) for removal. The
    // findings-store [WARN]-gate on null code_hash is what keeps degenerate
    // findings out of persistence and push payloads today.
    codeHash:     lineRaw ? crypto.createHash('sha256').update(lineRaw).digest('hex').slice(0, 32) : null,
    // findingId: content-based via lib/finding-identity.js (CLAUDE.md #16).
