...
function scanFileWithRules(filePath, content, rules, config, taintReg = null) {
  ...
  for (const rule of rules) {
    ...
    for (const match of matches) {
      ...
      // ── Taint-aware check ──────────────────────────────────────────────
      // If rule is taintAware, extract the variable name that appears after
      // the last concatenation operator (.) in the match line.
      // Check if that variable was assigned from external input earlier in
      // the file via the taint register. If the register has no match, skip.
      let taintInfo = null;
      if (rule.taintAware && taintReg) {
        // Extract variable name after last . operator — e.g. ". $id" → 'id'
        const varMatch = lineRaw.match(/\.\s*\$(?!_GET|_POST|_REQUEST|_COOKIE|_SESSION|stmt|pdo|conn|db)([a-zA-Z_]\w*)\s*[;,"'\)]/);
        ...
        // If no variable could be extracted, fall through and flag anyway
        // (better to have a FP than miss a TP in uncertain cases)
      }
