 ...
 */
function routeFinding(finding, fileContext, findings, suppressedFindings) {
...
async function scanFull(files, config = null, scope = null) {
  ...
  // Secrets scanner returns findings with filePath already set — build context
  // from filePath only (content not available at this stage; path signals suffice
  // for secrets findings which are typically in config/env files).
  for (const f of rawSecrets) {
    const fileContext = buildFileContext(f.filePath);
  ...
  for (const file of files) {
    ...
    // ── Build file context once per file ──────────────────────────────────
    // Built before the rule loop — shared across all rules for this file.
    // Content passed so Layer 2 (content signals) can run when path is ambiguous.
    const fileContext = buildFileContext(filePath, content);
    ...
    if (isLarge && noLimit) {
      ...
      if (rules) {
        for (const f of scanFileWithRules(filePath, content, rules, config, taintReg)) {
          routeFinding(f, fileContext, findings, suppressedFindings);
        }
      }
      if (exposureRules) {
        for (const f of scanFileWithRules(filePath, content, exposureRules, config, taintReg)) {
          routeFinding(f, fileContext, findings, suppressedFindings);
        }
      }
    }
    ...
    if (!INFRA_SKIP_EXTS.has(ext) && !INFRA_TEST_FILE_RE.test(filePath) && !isMinified && !isVendor) {
      for (const f of scanFileWithRules(filePath, content, INFRA_RULES, config)) {
        routeFinding(f, fileContext, findings, suppressedFindings);
      }
    }
