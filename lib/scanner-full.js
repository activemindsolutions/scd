...
function buildFinding(rule, filePath, lineNumber, snippet, lineRaw, config) {
  const finding = {
    ruleId:       rule.id,
    name:         rule.name,
...
function scanFileWithRules(filePath, content, rules, config) {
  const findings = [];
  ...
  for (const rule of rules) {
    ...
    for (const match of matches) {
      ...
      findings.push(buildFinding(rule, filePath, lineNumber, snippet, lineRaw, config));
    }
  }
