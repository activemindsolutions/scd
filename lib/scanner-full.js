...
function buildFinding(rule, filePath, lineNumber, snippet, lineRaw, config, taintInfo = null, matchObj = null) {
  ...
  const finding = {
    ...
    codeHash:     lineRaw ? makeCodeHash(lineRaw) : null,
...
function scanFileWithRules(filePath, content, rules, config, taintReg = null, commentMap = null) {
  ...
  for (const rule of applicableRules) {
    ...
    for (const match of matches) {
      ...
      // Display-only; strip terminal control sequences from the (untrusted) scanned line.
      // Identity (code_hash / finding_id) below uses lineRaw, so it is unaffected.
      const snippet    = stripControl(lineRaw.trim());
       ...
       if (rule.taintAware && taintReg) {
         ...
         if (strategy === 'interpolation') {
           ...
           if (mPhp1 || mPhp2) {
             ...
             const mPlus  = lineRaw.match(/[+]\s*([a-zA-Z_]\w{1,40})\s*[+,);'"`]/);
