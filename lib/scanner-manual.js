...
function discoverFiles(target, opts = {}) {
  ...
  if (stat.isDirectory()) {
    const found = walkDir(resolved, allowedExts);
...
function walkDir(dir, allowedExts) {
  const results = [];
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      results.push(...walkDir(fullPath, allowedExts));
