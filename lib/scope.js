...
function makePatternMatcher(pattern) {
  ...
  // Glob pattern. Convert to a regex: ** crosses path separators, a single *
  // matches within one segment, ? matches one non-separator char. Order matters —
  // handle ** before the remaining single * so ** is not seen as two * globs.
  if (pattern.includes('*') || pattern.includes('?')) {
    const regexStr = norm
      .replace(/[.+^${}()|[\]\\]/g, '\\$&')   // escape regex specials (not * ? — handled below)
      .replace(/\*\*/g, '__DOUBLESTAR__')     // ** first
      .replace(/\*/g, '[^/]*')                // remaining single * → within-segment glob
      .replace(/__DOUBLESTAR__/g, '.*')       // ** → cross-segment
      .replace(/\?/g, '[^/]');
    // Defensive: a malformed pattern must never crash a scan. Fail OPEN — treat it
    // as matching nothing, so a bad pattern hides no findings (mirrors the server's
    // scope-match.js hardening; keep the two matchers in lockstep).
    let re;
    try { re = new RegExp('^' + regexStr + '$'); }
    catch { return () => false; }
