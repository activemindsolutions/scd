...
// `display` can be attacker-controlled (a scanned file's name), so its terminal control
// sequences are stripped in both branches — otherwise a crafted name could break out of
// the OSC 8 envelope (`ESC \`) or drive the terminal. `uri` is built by the callers below
// via pathToFileURL, which percent-encodes control bytes.
function hyperlink(uri, display) {
  const safe = stripControl(display);
  if (!SUPPORTS_OSC8) return safe;
  return `${ESC}]8;;${uri}${ESC}\\${safe}${ESC}]8;;${ESC}\\`;
}
